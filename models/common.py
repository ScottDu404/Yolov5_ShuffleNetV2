...
# 注册GSConv_______________________________________________________
...
class CBRM(nn.Module):           #conv BN ReLU Maxpool2d
    def __init__(self, c1, c2):  # ch_in, ch_out
        ...
        self.conv = nn.Sequential(
            ...
            # nn.Conv2d(c1, c2, kernel_size=(1, 3), stride=2, padding=(0, 1), bias=False),
            # nn.Conv2d(c2, c2, kernel_size=(3, 1), stride=2, padding=(1, 0), bias=False),
            nn.BatchNorm2d(c2),
...
# 除去1*1卷积的Shuffle_Block
class Shuffle_Block_Lite(nn.Module):
    def __init__(self, ch_in, ch_out, stride):
        super(Shuffle_Block_Lite, self).__init__()
...
class SBLL(nn.Module):
    # Shuffle_Block_Lite_Layered,进行了卷积的重组
    def __init__(self, ch_in, ch_out, stride):
        super(SBLL, self).__init__()
        ...
        if self.stride > 1:
            self.branch1 = nn.Sequential(
                # self.depthwise_conv(ch_in, ch_in, kernel_size=3, stride=self.stride, padding=1),
                # nn.Conv2d(ch_in, ch_in, kernel_size=(1, 3), stride=self.stride, padding=(0, 1), bias=False, groups=1),
                self.depthwise_conv(ch_in, ch_in, kernel_size=(1, 3), stride=(1, self.stride), padding=(0, 1)),
                self.depthwise_conv(ch_in, ch_in, kernel_size=(3, 1), stride=(self.stride, 1), padding=(1, 0)),
                # self.depthwise_conv_restruct(ch_in, ch_in, kernel_size=3, stride=self.stride, padding=1),
                # nn.Conv2d(ch_in, ch_in, kernel_size=(2, 2), stride=self.stride, padding=1, bias=False),
                # self.depthwise_conv(ch_in, ch_in, kernel_size=2, stride=self.stride, padding=0),
                nn.BatchNorm2d(ch_in),
        ...
        self.branch2 = nn.Sequential(
            ...
            # self.depthwise_conv(branch_features, branch_features, kernel_size=3, stride=self.stride, padding=1),
            # nn.Conv2d(branch_features, branch_features, kernel_size=(1, 3), stride=self.stride, padding=(0, 1), bias=False, groups=1),
            # nice 终于好了
            self.depthwise_conv(branch_features, branch_features, kernel_size=(1, 3), stride=(1, self.stride), padding=(0, 1)),
            self.depthwise_conv(branch_features, branch_features, kernel_size=(3, 1), stride=(self.stride, 1), padding=(1, 0)),
            # self.depthwise_conv_restruct(branch_features, branch_features, kernel_size=3, stride=self.stride, padding=1),
            # nn.Conv2d(branch_features, branch_features, kernel_size=(2, 2), stride=self.stride, padding=1, bias=False),
            # self.depthwise_conv(branch_features, branch_features, kernel_size=2, stride=self.stride, padding=0),
