...
# https://blog.csdn.net/qq_43760302/article/details/128827322
...
if __name__ == "__main__":
    ...
    # 显示深度图像
        ...
        dim_gray = cv2.convertScaleAbs(dpt, alpha=0.05)
        # 对深度图像进行一种图像的渲染，目前有11种渲染方式，大家可以逐一去试下
        depth_colormap = cv2.applyColorMap(dim_gray, 5)  # 有0~11种渲染的模式
        cv2.imshow('depth', depth_colormap)
        # cv2.imshow('depth', dim_gray)
    ...
    #     cv2.imshow("ir", img)
        ...
        if capture_flag == 1:
            ...
            # cv2.imwrite(base_path + 'img/depth' + name + ".jpg", depth_colormap)
            # 要用png来保存，否则会丢失位数
            cv2.imwrite(base_path + 'img/depth' + name + ".png", dpt)
            cv2.imwrite(base_path + 'img/rgb' + name + ".png", cframe_data)
